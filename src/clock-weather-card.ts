...
import {
  ...
  handleAction,
  fireEvent, // <-- IMPORTANTE: aggiunto fireEvent
  TimeFormat,
  type ActionConfig
...
export class ClockWeatherCard extends LitElement {
  ...
  /**
   * Imposta la configurazione della card e aggiunge il parametro `tap_action`.
   */
  public setConfig (config?: ClockWeatherCardConfig): void {
    ...
    // Merge di tutta la config e in più gestiamo tap_action
    this.config = {
  ...
  protected render (): TemplateResult {
    ...
    return html`
      <ha-card
        @action=${(e: ActionHandlerEvent) => { this.handleTapAction(e) }}
      ...
      <clock-weather-card-today-right>
        <clock-weather-card-today-right-wrap>
          <clock-weather-card-today-right-wrap-top>
            ${this.config.hide_clock
              ? weatherString
              : localizedTemp
                ? `${weatherString}, ${localizedTemp}`
                : weatherString}
            ${this.config.show_humidity && localizedHumidity ? html`<br>${localizedHumidity}` : ''}
            ${this.config.apparent_sensor && apparentTemp
              ? html`<br>${apparentString}: ${localizedApparent}`
              : ''}
            ${this.config.aqi_sensor && aqi !== null
              ? html`<br><aqi style="background-color: ${aqiColor}">${aqi} ${aqiString}</aqi>`
              : ''}
          </clock-weather-card-today-right-wrap-top>
  ...
  private renderForecast (): TemplateResult[] {
    ...
    return forecasts.map((forecast, i) =>
      safeRender(() => this.renderForecastItem(
        forecast,
        gradientRange,
        minTemp,
        maxTemp,
  ...
  private renderForecastItem (
    ...
    const weatherState = forecast.condition === 'pouring'
      ? 'raindrops'
    ...
    const isNow = hourly
      ? DateTime.now().hour === forecast.datetime.hour
  ...
  private renderForecastCurrentTemp (
    minTempDay: number,
    maxTempDay: number,
    currentTemp: number | null
  ): TemplateResult {
    ...
    const indicatorPosition = minTempDay === maxTempDay
      ? 0
  ...
  private gradientRange (
    ...
    return Array.from(gradientMap.keys())
      ...
      .map((temp) => gradientMap.get(temp)!) // eslint-disable-line @typescript-eslint/no-non-null-assertion
  }
  ...
  /**
   * Nuovo metodo che gestisce il clic (tap) in base a `tap_action`.
   * Se `tap_action` non è configurato, di default apre il more-info.
   */
  private handleTapAction (ev: ActionHandlerEvent): void {
    if (!this.hass || !this.config || !ev.detail.action) return

    // Recuperiamo la configurazione di `tap_action`
    ...
    } else if (tapAction.action === 'navigate') {
  ...
  /**
   * (Facoltativo) Metodo precedente che gestiva le azioni hold/double_tap
   * Resta a disposizione se vuoi mantenerlo per hold_action / double_tap_action
   */
  ...
  private toIcon (weatherState: string, type: 'fill' | 'line', forceDay: boolean, kind: 'static' | 'animated'): string {
    const daytime = forceDay
      ? 'day'
      : this.getSun()?.state === 'below_horizon'
        ? 'night'
        : 'day'
  ...
  private getCurrentTemperature (): number | null {
    ...
    // Se non esiste un sensore esterno, usa la temperatura della weather entity
  ...
  private calculateBarRangePercents (
    minTemp: number,
    maxTemp: number,
    minTempDay: number,
    maxTempDay: number
    ...
    // sistemiamo possibili float imprecisi
    return {
  ...
  private mergeForecasts (maxRowsCount: number, hourly: boolean): MergedWeatherForecast[] {
    ...
    const agg = forecasts.reduce<Record<number | string, WeatherForecast[]>>(
      (forecasts, forecast) => {
  ...
  private determineForecastType (): 'hourly' | 'daily' | 'hourly_not_supported' {
    ...
    if (supportsDaily && supportsHourly) {
      ...
      // fallback
      console.warn(
        `clock-weather-card - Weather entity [${this.config.entity}] does not support daily forecast. Falling back to hourly forecast.`
      )
      return 'hourly'
    }
  }
